...
export default function WebGLHero({ fixed = false }) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className={`pointer-events-none ${fixed ? 'fixed inset-0 w-screen h-screen z-0 opacity-40' : 'absolute top-0 left-0 w-full h-full z-0 opacity-60'}`}
    />
  );
}
